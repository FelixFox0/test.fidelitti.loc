...
class ControllerExtensionModuleSeogenerator extends Controller {
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/seogenerator', 'token=' . $this->session->data['token'] . '&type=module', true)
   		);
	...
	public function generateSEFCategory() {	
		...
		$sql = $this->model_module_seogenerator->generateSEFCategories($this->request->get['selector'], $this->request->get['update']);
		$this->response->setOutput(json_encode($sql));
	}
	
	public function generateSEFProduct() {
		$this->load->model('extension/module/seogenerator');
		$sql = $this->model_module_seogenerator->generateSEFProducts($this->request->get['selector'], $this->request->get['update']);
		$this->response->setOutput(json_encode($sql));
	}
	
	public function generateSEFManufacturer() {
		$this->load->model('extension/module/seogenerator');
		$sql = $this->model_module_seogenerator->generateSEFManufacturers($this->request->get['selector'], $this->request->get['update']);
		$this->response->setOutput(json_encode($sql));
	}
	
	public function generateSEFInformation() {
		$this->load->model('extension/module/seogenerator');
		$sql = $this->model_module_seogenerator->generateSEFInformations($this->request->get['selector'], $this->request->get['update']);
		$this->response->setOutput(json_encode($sql));
	}
}
?>