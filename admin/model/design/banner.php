		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $language_id => $value) {
				foreach ($value as $banner_image) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image SET banner_id = '" . (int)$banner_id . "', language_id = '" . (int)$language_id . "', title = '" .  $this->db->escape($banner_image['title']) . "', title_two = '" .  $this->db->escape($banner_image['title_two']) . "', title_buttom = '" .  $this->db->escape($banner_image['title_buttom']) ."', link = '" .  $this->db->escape($banner_image['link']) . "', image = '" .  $this->db->escape($banner_image['image']) . "', sort_order = '" .  (int)$banner_image['sort_order'] . "'");
				}
			}
		}
	...
	public function getBannerImages($banner_id) {
		...
		foreach ($banner_image_query->rows as $banner_image) {
			$banner_image_data[$banner_image['language_id']][] = array(
				...
				'title_two'     => $banner_image['title_two'],//add input title two
				'title_buttom'  => $banner_image['title_buttom'],//add input title three
