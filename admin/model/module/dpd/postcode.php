...
class ModelModuleDpdPostcode extends Model
{
    ...
    public function getPathToDatabaseUpgradeFiles(){
        return  DIR_DOWNLOAD . DIRECTORY_SEPARATOR . 'dpd' . DIRECTORY_SEPARATOR . 'postcode_updates' . DIRECTORY_SEPARATOR;
    }
    ...
    public function updateDatabase($fileName){
        ...
        if(empty($result)){
            throw new Exception('An error occurred while updating postcode database. Please run again the import script. (A database backup is always created in zitec_dpd_postcodes_backup table.)');
        }
        return true;
    }
