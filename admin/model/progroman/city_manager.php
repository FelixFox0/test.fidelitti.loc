<?php
    class ModelProgromanCityManager extends Model {
        ...
        public function editCities($cities) {
            $query = "INSERT INTO prmn_cm_city (`fias_id`, `name`, `image`, `country_ua`, `country_ru`, `sort`) VALUES\n";
            $values = [];

            foreach ($cities as $city) {
                $values[] = "(" . (int)$city['fias_id'] . ", '" . $this->db->escape($city['name']) . "', '" . $this->db->escape($city['image']) . "', '" . $this->db->escape($city['country_ua']) . "' , '" . $this->db->escape($city['country_ru']) . "' , " . (int)$city['sort']  . ")";
            }
