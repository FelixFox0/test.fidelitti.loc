...
<div class="panel-heading">
    <h3 class="panel-title"><i class="fa fa-list"></i> <?php echo $language->get('Shipment Management'); ?></h3>
</div>
<div class="panel-body">
<ul class="nav nav-tabs">
    <li><a href="#tab-order" data-toggle="tab"><?php echo $tab_order; ?></a></li>
    <li class="active"><a href="#tab-shipment-management-orders" data-toggle="tab"><?php echo $tab_shipment_management; ?></a></li>
...
<div class="tab-pane active" id="tab-shipment-management-orders">
    <?php if(!empty($address_validation['errors']['postcode'])) { ?>
    <div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i>
        <?php $msg = $language->get('DPD shipping carrier detected an other postcode for current address. You can validate this postcode before scheduling deliveries by clicking the edit button or %s'); ?>
        <?php $link = '<a target="_blank" href="' . $url->link('sale/order/update', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL') .'">'.$language->get('here').'</a>'; ?>
        <?php echo sprintf($msg,$link); ?>
    </div>
    <?php } ?>
    <?php if(!empty($address_validation['errors']['street_length'])) { ?>
    <div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i>
        <?php echo $language->get('DPD shipping carrier requirements: The length of address provided by your customer is too long. Please provide a maximum 70 characters length address (street field).'); ?>
        <?php $link = '<a target="_blank" href="' . $url->link('sale/order/update', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL') .'">'.$language->get('here').'</a>'; ?>
        <?php echo sprintf('Click %s to edit the address information.',$link); ?>
    </div>
    <?php } ?>
    <?php if($isDpdShipping !== false ) { ?>
    <?php if (empty($shipment['dpd_shipment_reference_number'])) { ?>
    <h3><?php echo $language->get('Group the products in your shipment into parcels'); ?></h3>

    <p><?php echo $language->get('This module lets you organize your products into parcels using the table below. <b>If you like you can, leave some parcels empty, and the system will not take them into account.</b>'); ?></p>

    <p>

    <form id="form_parcels" name="form_parcels" action="<?php echo $index_shipment_url; ?>" method="post">
        <label>
            <?php echo $language->get('Total number of parcels'); ?>
            <input type="text" name="parcels_number" class="form-control" id="parcels_number" value="<?php echo $parcels_number; ?>">
        </label>
        <input type="submit" class="btn btn-default">
    </form>
    </p>

    <form id="form" name="form" action="<?php echo $create_shipment_url; ?>" method="post">
        <div class="table-responsive">
            <table class="table table-bordered table-hover">
            <thead>
            <tr>
                <td><?php echo $language->get('Product name'); ?></td>
                <td><?php echo $language->get('Model'); ?></td>
                <td><?php echo $language->get('Weight'); ?></td>
                <?php for ($j=1; $j<=$parcels_number; $j++) { ?>
                <td class="right"><?php echo $language->get('Parcel ').$j; ?></td>
                <?php } ?>
            </tr>
            </thead>
            <?php foreach ($products as $product) { ?>
            ...
            <tr>
                ...
                <td class="right"><?php echo round($product['product_data']['weight'],2); ?>
                    <input type="hidden" id="row_weight_<?php echo $product['product_id']; ?>_<?php echo $i; ?>"
                           name="row_weight_<?php echo $product['product_id']; ?>_<?php echo $i; ?>"
                           value="<?php echo round($product['product_data']['weight'],2); ?>"/>
                </td>
                <?php for ($j=1; $j<=$parcels_number; $j++) { ?>
                <td class="right">
                    <label class="parcel-radio-option-label">
                        <input type="radio"
                               class="parcel_radio_option" <?php if($j==1) { echo ' checked="checked" '; } ?>
                        name="parcels[<?php echo $product['product_id']; ?>_<?php echo $i; ?>]"
                        identifier="<?php echo $product['product_id']; ?>_<?php echo $i; ?>"
                        id="parcels_<?php echo $product['product_id']; ?>_<?php echo $i; ?>_<?php echo $j; ?>"
                        value="<?php echo $j; ?>" />
                    </label>
                </td>
                <?php } ?>
            ...
            <tfoot>
            <tr>
                <td colspan="2" class="text-left"><b><?php echo $language->get('Total Weight'); ?></b></td>
                <td class="right"><?php echo $total_weight; ?></td>
                <td class="right">
                    <input class="weight_input form-control" type="text" readonly name="total_weight_col_1" id="total_weight_col_1"
                           value="<?php echo $total_weight; ?>"/>
                </td>
                <?php for ($j=2; $j<=$parcels_number; $j++) { ?>
                <td>
                    <input class="weight_input form-control" type="text" readonly name="total_weight_col_<?php echo $j; ?>"
                           id="total_weight_col_<?php echo $j; ?>"/>
                </td>
                <?php } ?>
            </tr>
            </tfoot>
        ...
        <div>
            <table class="table table-bordered table-hover">
                <tr>
                    <td class="right"><?php echo $language->get('Cost of shipping to customer (excluding tax):'); ?></td>
                    <td id="shipping_cost_paid">
                        <?php  foreach ($totals as $total) { ?>
                        <?php if( $total['code']=='shipping' ) { echo  $total['text']; } ?>
                        <?php } ?>
                    </td>
                </tr>
                <tr>
                    <td class="right"><?php echo $language->get('Shipping cost (excluding tax):'); ?></td>
                    <td id="shipping_cost"></td>
                </tr>
                <tr>
                    <td class="right"><?php echo $language->get('Difference:'); ?></td>
                    <td id="shipping_profit"></td>
                </tr>
                <tr>
                    <td colspan="2">
                        <a onclick="$('#form').submit();" target="_blank" class="btn btn-default right"><?php echo $language->get('Create'); ?></a>
                    </td>
                </tr>
            </table>
        </div>
    ...
    <div>
        <table class="table table-bordered table-hover" >
            <tbody>
            <tr>
                <td class="left"><?php echo $language->get('Shipment'); ?></td>
                <td class="left"><a href="<?php echo $print_shipments_url;?>"><?php echo $shipment['dpd_shipment_reference_number']; ?></a>
                </td>
            </tr>
            <tr>
                <td class="left"><?php echo $language->get('Track this shipment'); ?></td>
                <td><a href="<?php echo $traking_url; ?>" target="_blank"><?php echo $traking_url; ?></a></td>
            </tr>
            </tbody>
        </table>
    </div>
    ...
    <?php  } ?>
    <?php  } else { ?>
    <div><?php echo $language->get('No information available in this section. '); ?></div>
    <?php } ?>
</div>
...
<script type="text/javascript"><!--
    $( document ).ready(function() {
        ...
        $('.parcel_radio_option').click(function (e) {
            calculateShippingPriceDPD();
        });
        if ($('.create_shipment_button').length >= 1) {
            calculateShippingPriceDPD();
        }

    });

    function calculateShippingPriceDPD(){
        $('.ajax-warning').fadeOut();
        $('.ajax-warning').remove();
        $('.content').loadingOverlay();
        computeParcelsWeight();
        var url = '<?php echo $validate_shipment_url; ?>';
        $.ajax({
            type: "POST",
            url: url,
            data: $('#form').serialize(),
            success: function (response) {
                ...
                if (objResponse.error == false) {
                    $('#shipping_cost').html(objResponse.shippingcost);
                    $('#shipping_profit').html(objResponse.profit);
                    $('#shipping_cost_paid').html(objResponse.price_paid);
                    $('#shipping_profit').css('color', objResponse.profitcolor);
                } else {
                    var error = '<div class="warning ajax-warning">' + objResponse.error + '</div>';
                    $('#tab-shipment-managemenet-orders').prepend(error);
                }
            }
        });
    ...
    function computeParcelsWeight() {
        var parcelWeight = [];

        $('.parcel_radio_option').each(function (i, el) {
            if ($(el).is(':checked') == false) {
                if (parcelWeight[$(el).val()] == undefined) {
                    parcelWeight[$(el).val()] = 0;
                }
                return true;
            }
            if (parcelWeight[$(el).val()] == undefined) {
            ...
            parcelWeight[$(el).val()] = parseFloat(parcelWeight[$(el).val()]) + parseFloat($('#row_weight_' + identifier).val());

        });
        for (var i in parcelWeight) {
            $('#total_weight_col_' + i).val(parcelWeight[i]);
        }
