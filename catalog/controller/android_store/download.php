<?php
class ControllerAndroidStoreDownload extends Controller {
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('android_store/download', '', true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('android_store/download', '', true)
		);
		...
		$results = $this->model_account_download->getDownloads(($page - 1) * $this->config->get($this->config->get('config_theme') . '_product_limit'), $this->config->get('config_product_limit'));
		...
		foreach ($results as $result) {
			if (file_exists(DIR_DOWNLOAD . $result['filename'])) {
				...
				$data['downloads'][] = array(
					...
					'href'       => $this->url->link('android_store/download/download', 'download_id=' . $result['download_id'], true)
				);
			}
		}
		...
		$pagination->url = $this->url->link('android_store/download', 'page={page}', true);
		...
		$this->response->setOutput($this->load->view('android_store/download', $data));
	}
	...
	public function download() {
		...
		if ($download_info) {
			...
			$this->response->redirect($this->url->link('android_store/download', '', true));
		}
	}
}