<?php
class ControllerAndroidStoreReward extends Controller {
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('android_store/reward', '', true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('android_store/reward', '', true)
		);
		...
		foreach ($results as $result) {
			$data['rewards'][] = array(
				...
				'href'        => $this->url->link('android_store/order/info', 'order_id=' . $result['order_id'], true)
			);
		}
		...
		$pagination->limit = 10;
		$pagination->url = $this->url->link('android_store/reward', 'page={page}', true);
		...
		$this->response->setOutput($this->load->view('android_store/reward', $data));
	}
}