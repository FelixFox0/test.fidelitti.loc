<?php
class ControllerAndroidStoreTransaction extends Controller {
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('android_store/transaction', '', true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('android_store/transaction', '', true)
		);
		...
		$pagination->limit = 10;
		$pagination->url = $this->url->link('android_store/transaction', 'page={page}', true);
		...
		$this->response->setOutput($this->load->view('android_store/transaction', $data));
	}
}