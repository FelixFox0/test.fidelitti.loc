...
class Controllerproductquickview extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['text_tk'] = $this->language->get('text_tk');
			...
			$data['full'] = 'image/' . $product_info['image'];
		} else {
			$data['full'] = '';
		}	
			...
			if ($product_info['image']) {
				$data['thumb'] = $this->model_tool_image->resize($product_info['image'], 50, 50);
			} else {
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'thumb' => $this->model_tool_image->resize($result['image'], 390, 706)
				);
			}

			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				$data['price'] = $this->currency->format($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
				$data['price_roznica'] = $this->currency->format($this->tax->calculate($product_info['price'] * 2 , $product_info['tax_class_id'], $this->config->get('config_tax')));
			} else {
				$data['price'] = false;
				$data['price_roznica'] = false;
			}
			...
			if ((float)$product_info['special']) {
				...
				$data['special_roznica'] = $this->currency->format($this->tax->calculate($product_info['special'] * 2, $product_info['tax_class_id'], $this->config->get('config_tax')));
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/quick_view.tpl')) {
				...
				$this->response->setOutput($this->load->view('/product/quick_view.tpl', $data));
			}
		} else {
			$url = '';
