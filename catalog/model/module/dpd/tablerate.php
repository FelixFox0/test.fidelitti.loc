...
class ModelModuleDpdTablerate extends Model
{
    ...
    public function getRatesByRequest($request)
    {
        if (isset(
            ...
            $query = "SELECT * FROM " . DB_PREFIX . "zitec_dpd_tablerate  WHERE
                    ...
                    ORDER BY condition_threshold DESC, dest_country DESC, dest_region DESC, dest_postcode DESC, dpd_service DESC
            ...
            if (count($query->rows)) {
                ...
                foreach ($query->rows as $rate) {
                    if (empty($rows[$rate['dpd_service']])) {
                        $rows[$rate['dpd_service']] = $rate;
                    }
                }
    ...
    {
        $default_cod_surcharge_calculation = $this->_getSettingsModel()->getFrontendSettings('cod_surcharge_calculation', 'payment');
        $default_cod_surcharge_amount      = $this->_getSettingsModel()->getFrontendSettings('cod_surcharge_amount', 'payment');
        $default_cod_surcharge_min_amount  = $this->_getSettingsModel()->getFrontendSettings('cod_surcharge_min_amount', 'payment');
        ...
        $sampleRate = array(
            ...
            'cod_surcharge_calculation'  => $default_cod_surcharge_calculation,
            'cod_surcharge_amount'       => $default_cod_surcharge_amount,
            'cod_surcharge_min_amount'   => $default_cod_surcharge_min_amount,
        );
