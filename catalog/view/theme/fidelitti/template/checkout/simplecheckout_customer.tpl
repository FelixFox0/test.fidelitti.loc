<div class="simplecheckout-block" id="simplecheckout_customer" <?php echo $hide ? 'data-hide="true"' : '' ?> <?php echo $display_error && $has_error ? 'data-error="true"' : '' ?>>
  ...
  <div class="checkout-heading panel-heading"><?php if ($display_login) { ?><div style="font-size: 14px;margin-bottom: 12px;">Войдите в аккаунт для ускорения процесса оформление заказа</div><span class="checkout-heading-button"><a href="javascript:void(0)" data-onclick="openLoginBox"><?php echo $text_checkout_customer_login ?></a></span><?php } ?></div>
  <?php } ?>
