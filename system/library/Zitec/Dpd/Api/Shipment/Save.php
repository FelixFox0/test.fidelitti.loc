...
class Zitec_Dpd_Api_Shipment_Save extends Zitec_Dpd_Api_Shipment
{
    ...
    /**
     *
     * @param $address - array containing $street, $city, $postcode, $countryId
     *
     * @return \Zitec_Dpd_Api_Shipment_Save
     */
    public function setReceiverAddress($address)
    {
        ...
        $countryId       = $address['iso_code_2'];
        $customer        = $address['customer'];
        $telephone       = $address['telephone'];
        $payment_company = $address['payment_company'];

        //in this case the field is not needed to be processed automatically, the store admin have already processed manually
        //substr(implode(explode("\n", $street)), 0, Zitec_Dpd_Api_Configs::SHIPMENT_LIST_RECEIVER_STREET_MAX_LENGTH - 1)
        $processedStreet = $street;
        $this->setShipmentList(Zitec_Dpd_Api_Configs::SHIPMENT_LIST_RECEIVER_NAME, $customer);
        $this->setShipmentList(Zitec_Dpd_Api_Configs::SHIPMENT_LIST_RECEIVER_FIRM_NAME, $payment_company);
        ...
        $this->setShipmentList(Zitec_Dpd_Api_Configs::SHIPMENT_LIST_RECEIVER_PHONE_NO, $telephone);
    ...
    protected function _setParcelId($parcelReferenceNumber, $parcelId)
    {
        ...
        foreach (array_keys($parcels) as $parcelKey) {
            ...
            if(isset($parcels[$parcelKey][Zitec_Dpd_Api_Configs::PARCELS_PARCEL_REFERENCE_NUMBER])) {
