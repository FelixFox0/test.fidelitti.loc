...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['button_pred_order'] = $this->language->get('button_pred_order');
...
$data['weight'] = number_format($product_info['weight'], 0).' '.$this->weight->getUnit($product_info['weight_class_id']);
			...
			if ($product_info['image']) {
				$data['images'][] = array(
					'popups' => 'image/' . $product_info['image'],
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'popups' => $this->model_tool_image->resize($result['image'], $this->config->get($this->config->get('config_theme') . '_image_related_width'), $this->config->get($this->config->get('config_theme') . '_image_related_height')),
...
//BOF Product Series 
			...
			foreach ($results as $result)
			{
				...
				$data['pds'][] = array(
					...
					'price' => utf8_substr($result['price'], 0, -5),
					'upc' => $result['upc'],
